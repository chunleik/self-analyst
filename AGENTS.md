...
README 按语言版本分别维护，以下规则优先于“文档默认使用简体中文”的通用约定：

- **`README.md` 是 GitHub 默认展示的英文首页，必须保持英文。严禁直接追加中文功能说明、中文标题或中文段落，即使原始需求、OpenSpec 或实现说明使用中文，也必须先翻译为英文再写入。**
- **`README.zh-CN.md` 是简体中文版。** 涉及 README 的功能、用法或行为变化时，必须同步更新两个版本，保持信息一致，并分别使用各自的语言。
- 语言要求覆盖标题、正文、列表、表格、链接说明及示例中的解释性文字。中文版本入口（如“简体中文”）、必要的专有名词或必须原样展示的输入输出不受此限制，但不得借此保留整段中文功能介绍。
- 修改 README 后、提交前，必须检查完整差异和新增段落，确认英文版没有混入中文说明，中文版没有遗漏对应更新；尤其检查文件末尾追加的内容。

除上述 README 语言版本外，项目说明性文档默认使用简体中文，包括 `docs/` 下的规范文档、设计文档、API 文档、用户文档及 OpenSpec 产物。命令、代码标识符、文件路径、协议名称、产品名称及其他不宜翻译的专有名词可以保留英文。
