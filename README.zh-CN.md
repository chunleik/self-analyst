...
桌面端默认进入“看板”，以全宽时间轴回顾当前、今天和历史时段的活动。点击条目可展开详情，
选择“继续追问”可带着该条目的上下文进入会话。看板不再显示顶部建议卡片和右侧未来任务栏；
会话中的任务上下文与任务生成功能继续保留。
...
桌面标题栏在 SelfAnalyst 名称旁提供「帮助」，包含「使用指南」「反馈问题」和「关于 SelfAnalyst」。
...
点击“添加图片”选择 PNG/JPEG 文件，或在会话输入框内粘贴截图。发送前可以预览和移除图片，
支持带问题一起发送或仅发送图片；“发送图文”和 Enter 都会提交当前文字与图片。
增强界面和降级界面使用相同流程。每轮最多 4 张，每张不超过 5 MiB、2000 万像素，合计最多 20 MiB。

图片随会话保存在本地，发送时会交给当前配置的对话模型，模型需要支持图片输入。
模型明确拒绝图片时，可切换视觉模型后重试原轮次。刷新或重启会恢复历史图片，删除会话会同时清理
