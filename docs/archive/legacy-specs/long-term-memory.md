...
实施本特性前，SelfAnalyst 已有 `MemoryStore` 与 `GrowthProfile`，将 `memory.json` 作为用户长期档案。既有模型包含 `goals`、`patterns`、`logs`，并在 Agent 系统提示词中作为上下文使用。

当时的限制：

- `GrowthProfile` 主要是静态档案，普通聊天不会自动写回有价值的信息；
- 桌面端没有记忆候选确认、手动添加、停用和删除入口；
- 会话持久化已经提供消息正文与摘要，但尚未把会话内容转化为长期用户画像、目标或偏好。
...
| SPEC-LTM-PROMPT-001..005 | `agent/SelfAnalystAgent.java`、`agent/DynamicMemoryContextMiddleware.java`、`agent/AgentPrompts.java`、`agent/PromptResources.java`、`resources/prompts/agent/memory-*.md`、`memory/GrowthProfile.java` | `AgentPromptsTest`、`PromptResourcesTest`、`DynamicMemoryContextMiddlewareTest` |
