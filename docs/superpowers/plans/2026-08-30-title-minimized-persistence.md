...
> 历史说明：本文记录标题最小化改造当时的范围判断。OCR 与声音模块随后已于 2026-08-30 暂时移除；
> 当前契约和恢复说明以 `docs/specs/removed-ocr-audio.md` 为准，本文不再作为现行功能清单。

> 后续修订：原方案曾把文件正文临时读取、摘要与语义索引列为例外。用户随后明确取消该例外；
> 现行文件契约以 `docs/specs/file.md` 的严格元数据边界为准，文件采集器不得读取或外发正文。
...
6. 文件模块原先允许临时读取正文；该决策已被后续严格元数据边界取代，现行实现禁止正文读取、哈希、摘要和内容向量。
...
- 文件模块现已由后续修订改为严格元数据采集；本条原“文件正文例外”不再有效。
...
本方案实施时 `FileIndexWorker` 曾读取正文并生成摘要/向量；该范围判断已被后续严格元数据边界推翻。现行实现只读取 `BasicFileAttributes`，旧派生字段与 Lucene 文件索引会被净化。
...
本节记录旧方案，现已由 `docs/specs/file.md` 取代。文件正文读取、摘要、内容哈希和内容 embedding 已全部取消：

- `file-watch.db` v2 只包含路径、大小、创建/修改/采集时间和运行状态。
- `FileRecord` 类型无法表达正文、哈希、摘要、主题、prompt、模型或 embedding。
- 旧数据库执行表重建与 `VACUUM`，旧 Lucene 文件内容索引在启动时清理。
- FileTools 只在本地按文件名、路径和时间查询。

数据库、WAL、SHM 和 worker 字节码测试证明文件采集链路不存在正文读取与正文副本。
...
| 测试 | 预期 |
|------|------|
| 使用带唯一秘密标记的文件完成采集 | worker 不打开正文流，`file-watch.db`/WAL/SHM 不含秘密标记 |
| 打开旧 v1 数据库 | 重建为 metadata-only v2，并通过 `VACUUM` 清除旧摘要/主题 |
| 启动旧文件 Lucene 索引目录 | 删除旧索引产物，且根目录保护测试通过 |
...
8. 文件采集器不读取正文；`file-watch.db` v2 不含哈希、摘要、主题等正文派生字段，旧文件语义索引已净化。
