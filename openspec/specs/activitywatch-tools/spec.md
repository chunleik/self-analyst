...
定义 Agent 通过本地 ActivityWatch HTTP API 列出 bucket、查询事件、执行 AQL 和读取服务器信息的请求、预算与错误降级行为。
...
内部辅助方法和构造器 MUST NOT 作为 Agent 工具暴露。

#### Scenario: Toolkit 注册
- **WHEN** SelfAnalystAgent 注册 ActivityWatchTools
