...
Agent 的 ActivityWatch 查询 SHALL 通过注册的 ActivityWatchTools；窗口、AFK、标题与文件 heartbeat 等
采集链 MAY 直接调用本地 AW HTTP API。

#### Scenario: Agent 查询 ActivityWatch
- **WHEN** Agent 需要列出 bucket、查询事件或执行 AQL
- **THEN** 调用通过 ActivityWatchTools 发往配置的 AW base URL，不直接访问数据库
...
Agent SHALL 注册 ActivityWatchTools，并在相应依赖可用时注册 ConfigTools、WikiTools、FileTools 和
