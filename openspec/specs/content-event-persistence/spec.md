...
Wiki 从原始 ActivityWatch 内容事件构造事实时 SHALL 只读取窗口标题、上下文标题、应用和上下文种类，
MUST NOT 读取 `text_content`。更高层级 Wiki 聚合 MAY 消费子级 metrics、summary 和 primaryTask，
并 MAY 持久化由标题事实及这些既有派生结果生成的摘要、主要任务和任务片段。
...
系统 SHALL 对 ID 以 `aw-watcher-content_` 开头，或已存 bucket 的 `client` 大小写不敏感等于
`aw-watcher-content` 的所有应用写入执行内容事件 v2 策略；其他 bucket 的既有通用字段 SHALL 不受
此策略影响。

#### Scenario: bucket ID 前缀触发策略
- **WHEN** 禁止字段写入 ID 以 `aw-watcher-content_` 开头的 bucket
- **THEN** 写入被内容事件策略拒绝

#### Scenario: bucket client 触发策略
- **WHEN** 禁止字段写入自定义 ID、但 client 为 `aw-watcher-content` 的已存 bucket
- **THEN** 写入被内容事件策略拒绝
...
嵌入式启动迁移 SHALL 扫描 bucket ID 具有内容前缀，或 bucket client 精确等于规范值
`aw-watcher-content` 的内容行，并将其规范化为 v2 投影，删除正文和未知字段。首次运行 SHALL 扫描
...
- **WHEN** 自定义 bucket 的 client 仅以大小写变体匹配 `aw-watcher-content`，且 bucket ID 没有内容前缀
