...
原始事实 SHALL 来自当前主机的 window、AFK 和 content bucket。窗口事件 SHALL 用于应用耗时、切换
次数和窗口标题样本；AFK 用于非活跃时间；content 只提供 app、系统标题、可选上下文标题和种类。
任一 bucket 缺失或查询失败时，其他可用事实 MAY 继续生成。单次 prompt 输入 SHALL 受
`wiki.prompt.maxContentChars` 限制，单个窗口标题样本默认最多 160 字符；上下文标题 SHALL 按
app、effectiveTitle、contextKind 去重。Wiki MUST NOT 读取 `text_content`。

#### Scenario: content bucket 含旧正文
- **WHEN** 内容事件同时具有标题字段和旧 text_content
- **THEN** Wiki facts 只使用标题投影，prompt 和数据库不包含旧正文

#### Scenario: 部分 bucket 缺失
- **WHEN** AFK 或 content bucket 不存在，但 window bucket 有可用事件
