...
public final class SupportedKeys {
    ...
    static {
        ...
        describe("events.raw.dir", "永久原始事件分区和 catalog 的保存目录。", "Directory for permanent raw-event partitions and their catalog.");
        describe("events.raw.query.maxRangeDays", "单次原始事件查询允许的最大时间范围（天，最多 366）。", "Maximum time range allowed for one raw-event query, in days (up to 366).");
        describe("events.raw.query.maxPageSize", "单页原始事件查询允许返回的最大事件数（最多 10000）。", "Maximum number of raw events returned on one page (up to 10000).");
        describe("events.raw.lowDisk.warnBytes", "原始目录可用空间低于此字节数时发出告警。", "Warn when free space for the raw directory falls below this byte count.");
        describe("events.raw.lowDisk.blockBytes", "原始目录可用空间低于此字节数时阻止新采集。", "Block new collection when free space for the raw directory falls below this byte count.");
        describe("events.raw.integrity.startupScope", "启动完整性校验范围：latest 或 all。", "Startup integrity verification scope: latest or all.");
        describe("events.raw.projector.batchSize", "原始事件投影器单批处理的最大事件数（最多 10000）。", "Maximum number of raw events processed in one projector batch (up to 10000).");
