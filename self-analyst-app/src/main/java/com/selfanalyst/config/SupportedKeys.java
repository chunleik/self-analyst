...
public final class SupportedKeys {
    ...
    static {
        ...
        put("events.mode", "embedded", KeyType.STRING);
        put("events.port", "5700", KeyType.INTEGER);
        put("events.base-url", "http://localhost:5700/api/0", KeyType.STRING);
        put("events.timeout", "15000", KeyType.INTEGER);
        put("events.data-dir", "./data/events", KeyType.STRING);
        put("events.export.maxRangeDays", "31", KeyType.INTEGER);
        put("events.storage.lowDisk.warnBytes", "10737418240", KeyType.INTEGER);
        put("events.storage.lowDisk.blockBytes", "1073741824", KeyType.INTEGER);
        describe("events.export.maxRangeDays", "合并事件等数据导出的最大时间范围（天）。", "Maximum range for merged-event and other data exports, in days.");
        describe("events.storage.lowDisk.warnBytes", "活动库剩余空间告警阈值（字节）。", "Free-space warning threshold for the active event store, in bytes.");
        describe("events.storage.lowDisk.blockBytes", "活动库剩余空间阻断阈值（字节）。", "Free-space threshold that blocks new event writes, in bytes.");
        put("events.raw.dir", "./data/events/raw", KeyType.STRING);
        put("events.raw.query.maxRangeDays", "31", KeyType.INTEGER);
        put("events.raw.query.maxPageSize", "1000", KeyType.INTEGER);
        put("events.raw.lowDisk.warnBytes", "10737418240", KeyType.INTEGER);
        put("events.raw.lowDisk.blockBytes", "1073741824", KeyType.INTEGER);
        put("events.raw.integrity.startupScope", "latest", KeyType.STRING);
        put("events.raw.projector.batchSize", "1000", KeyType.INTEGER);
        put("events.collection.window", "true", KeyType.BOOLEAN);
        put("events.collection.afk", "true", KeyType.BOOLEAN);
        put("events.collection.title.enabled", "true", KeyType.BOOLEAN);
        ...
        describe("events.mode", "事件服务运行模式：embedded 或 external。", "Event service mode: embedded or external.");
        describe("events.port", "内嵌事件服务监听端口。", "Listening port for the embedded event service.");
        describe("events.base-url", "事件服务 HTTP API 基础地址。", "Base URL of the event service HTTP API.");
        describe("events.timeout", "事件服务 HTTP 请求超时时间（毫秒）。", "Timeout for event service HTTP requests, in milliseconds.");
        describe("events.data-dir", "事件服务数据文件目录。", "Directory for event service data files.");
        describe("events.raw.dir", "已退役；仅 raw.dir 保留用于旧格式迁移。", "Retired; only raw.dir remains in use for legacy migration.");
        describe("events.raw.query.maxRangeDays", "已退役；仅 raw.dir 保留用于旧格式迁移。", "Retired; only raw.dir remains in use for legacy migration.");
        describe("events.raw.query.maxPageSize", "已退役；仅 raw.dir 保留用于旧格式迁移。", "Retired; only raw.dir remains in use for legacy migration.");
        describe("events.raw.lowDisk.warnBytes", "已退役；仅 raw.dir 保留用于旧格式迁移。", "Retired; only raw.dir remains in use for legacy migration.");
        describe("events.raw.lowDisk.blockBytes", "已退役；仅 raw.dir 保留用于旧格式迁移。", "Retired; only raw.dir remains in use for legacy migration.");
        describe("events.raw.integrity.startupScope", "已退役；仅 raw.dir 保留用于旧格式迁移。", "Retired; only raw.dir remains in use for legacy migration.");
        describe("events.raw.projector.batchSize", "已退役；仅 raw.dir 保留用于旧格式迁移。", "Retired; only raw.dir remains in use for legacy migration.");
        describe("events.collection.window", "是否采集活动窗口与应用信息。", "Whether to collect active-window and application information.");
        describe("events.collection.afk", "是否采集用户离开/活跃状态。", "Whether to collect user AFK/active status.");
        describe("events.collection.title.enabled", "是否识别并保存活动窗口的上下文标题；不会保存 UIA 原始正文。", "Whether to identify and save active-window context titles without persisting raw UIA body text.");
